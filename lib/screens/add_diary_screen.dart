...
class _AddDiaryScreenState extends State<AddDiaryScreen> {
  ...
  late List<Picture> _initialPictures;
  late bool _initialIsLocked;
  ...
  void initState() {
    ...
    _initialPictures = List.from(_pictures);
    _initialIsLocked = _isLocked;
  }
      ...
      if (!await _lockService.verifyPin(pin, pinPromptMode: pinPromptMode)) {
        if (mounted) {
          ScaffoldMessenger.of(
            context,
          ).showSnackBar(const SnackBar(content: Text('PIN 錯誤')));
        }
        return;
      }
    } else {
  ...
  bool get _hasUnsavedChanges {
    if (_isLocked != _initialIsLocked) return true;
    if (!_isSameCalendarDay(_selectedDate, _initialSelectedDate)) return true;
    if (_contentController.text != _initialContent) return true;
    return !_pictureListsEqual(_pictures, _initialPictures);
  }
  ...
  bool _pictureListsEqual(List<Picture> first, List<Picture> second) {
    if (first.length != second.length) return false;
    for (var i = 0; i < first.length; i++) {
      final left = first[i];
      final right = second[i];
      if (left.pictureUrl != right.pictureUrl ||
          left.caption != right.caption ||
          left.isLocalFile != right.isLocalFile ||
  ...
  Future<void> _editContent() async {
    final result = await Navigator.push<String>(
      ...
      MaterialPageRoute(
        builder: (context) =>
            ContentEditScreen(initialContent: _contentController.text),
      ),
    );
      ...
      try {
        if (widget.diary != null) {
          ...
          if (mounted) {
            ScaffoldMessenger.of(
              context,
            ).showSnackBar(const SnackBar(content: Text('日記已存至本機，背景同步中')));
          }
        } else {
  ...
  bool _isContentOverflowing(String text) {
    ...
    final TextPainter textPainter =
        TextPainter(
          ...
          maxWidth: MediaQuery.of(context).size.width - 32 - 24,
        ); // 减去padding和icon宽度
